...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try{
            ...
            db.execSQL("create table if not exists account(id INTEGER PRIMARY KEY AUTOINCREMENT, accounttype VARCHAR, section VARCHAR, name VARCHAR, username VARCHAR, password VARCHAR, v_date VARCHAR, email VARCHAR, contact VARCHAR, address VARCHAR)");

            db.execSQL("create table if not exists subject(s_id INTEGER PRIMARY KEY AUTOINCREMENT, s_name VARCHAR, room VARCHAR, days VARCHAR, time VARCHAR, a_id VARCHAR, lessons VARCHAR)");

            db.execSQL("create table if not exists class(c_id INTEGER PRIMARY KEY AUTOINCREMENT, s_name VARCHAR, a_id VARCHAR)");

            db.execSQL("create table if not exists homework(h_id INTEGER PRIMARY KEY AUTOINCREMENT, a_id VARCHAR, s_name VARCHAR, instruction VARCHAR, d_date VARCHAR, time VARCHAR, status VARCHAR, score VARCHAR, item VARCHAR)");

            db.execSQL("create table if not exists finish(f_id INTEGER PRIMARY KEY AUTOINCREMENT, h_id VARCHAR, a_id VARCHAR, instruction VARCHAR, answer VARCHAR, c_date VARCHAR, c_time VARCHAR, score VARCHAR, item VARCHAR)");
        ...
        try{
            ...
            if(c.getCount() > 0){
                ...
                if(strAccounttype.equalsIgnoreCase("admin")){
                    ...
                    ab.setTitle("NOTICE");
                    ab.setMessage("Welcome to MTC, "+strName+"!");
