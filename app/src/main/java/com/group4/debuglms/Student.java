...
public class Student extends AppCompatActivity
    ...
    int late,incoming,submitted, late_submitted;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvName5 = (TextView)nv.findViewById(R.id.tvName5);
        ...
        try{
            ...
            String c_date = new SimpleDateFormat("MM-dd-yyyy HH:mm", Locale.getDefault()).format(new Date());
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        int id = item.getItemId();
         if (id == R.id.nav_studentProfile) {
            String ct = "Student Profile";
            Intent i = new Intent(getApplicationContext(), ViewStudentProfile.class);
            i.putExtra("id",strId);
            ...
            try {
                ...
                String[] status = {"late", "incoming","submitted","late_submitted"};
                ...
                for (int j = 0; j < status.length; j++) {
                    ...
                    } else if (status[j].equalsIgnoreCase("late_submitted")) {
                        late_submitted += c.getCount();
                    }
                }
                ...
                statuss.add("\t\t\t\t\t\t\t\t\t\t\t\t\t\t"+late + " Pass Due Date");
                statuss.add("\t\t\t\t\t\t\t\t\t\t\t\t\t\t"+incoming + " Incoming");
                statuss.add("\t\t\t\t\t\t\t\t\t\t\t\t\t\t"+submitted + " Submitted");
                statuss.add("\t\t\t\t\t\t\t\t\t\t\t\t\t\t"+late_submitted + " Late Submitted");
                ...
                lvStatus.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                        ...
                        if(i == 0){
                            ...
                            if(late_submitted == 0){
                                Toast.makeText(getApplicationContext(), "No Late Submitted Homework!", Toast.LENGTH_SHORT).show();
                            }else{
                                strStatus = "late_submitted";
                                Intent j = new Intent(getApplicationContext(), HomeworkList.class);
                                j.putExtra("id",strId);
                                j.putExtra("status",strStatus);
                                j.putExtra("name",strName);
